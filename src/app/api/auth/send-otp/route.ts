...
import { users as dbUsers, notifications as dbNotifications, otps as dbOtps } from '@/src/db/schema';
import { eq, sql } from 'drizzle-orm';
import { ensureDbSeeded, otps as memoryOtps, getTransporter, broadcastSse } from '@/src/lib/server-backend';
...
export async function POST(req: NextRequest) {
  try {
    ...
    // Save in DB table and memory fallback
    try {
      await db.delete(dbOtps).where(eq(dbOtps.email, normalizedEmail));
      await db.insert(dbOtps).values({
      ...
      console.warn('Database OTP insertion fallback to memory:', dbErr);
    }
    memoryOtps.set(normalizedEmail, code);
    ...
    if (mailTransporter) {
      try {
        ...
        isSmtpSent = true;
        console.log(`[SMTP DISPATCH SUCCESS] Real OTP email dispatched to ${normalizedEmail}`);
      } catch (mailError) {
        console.error('[SMTP DISPATCH ERROR] Failed to send real email via SMTP, falling back...', mailError);
      }
    } else {
      console.log(`[CONSOLE FALLBACK] SMTP settings not configured. OTP verification code ${code} printed to console for ${normalizedEmail}`);
    }
    ...
    return NextResponse.json({
      success: true,
      message: isSmtpSent
        ? `Verification code successfully dispatched to ${normalizedEmail} (via Real SMTP)`
        : `Verification code successfully dispatched to ${normalizedEmail}`,
    });
  } catch (error) {
