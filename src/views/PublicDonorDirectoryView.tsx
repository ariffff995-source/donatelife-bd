...
interface PublicDonorDirectoryViewProps {
...
export default function PublicDonorDirectoryView({ currentUser }: PublicDonorDirectoryViewProps) {
  ...
  const handleToggleFavorite = async (donorId: string, e: React.MouseEvent) => {
    e.stopPropagation();
    if (!currentUser) {
      showToast(
        language === 'bn' ? 'প্রিয় তালিকায় যোগ করতে লগইন করুন।' : 'Please login to save favorite donors.',
        'error'
      );
      return;
    }
    ...
    try {
      const res = await fetch('/api/donors/favorite', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ favoriteDonorId: donorId })
      }).then(r => r.json());

      if (res.favorited) {
        setFavorites(prev => [...prev, donorId]);
        showToast(
          language === 'bn' ? 'প্রিয় তালিকায় যোগ করা হয়েছে ❤️' : 'Added to favorite donors ❤️',
          'success'
        );
      } else {
        setFavorites(prev => prev.filter(id => id !== donorId));
        showToast(
          language === 'bn' ? 'প্রিয় তালিকা থেকে সেশন সরানো হয়েছে' : 'Removed from favorite donors',
          'success'
        );
      }
    } catch (err) {
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-10 space-y-8 relative">
      ...
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-4 text-left">
        ...
        <div className="flex items-center gap-2">
          <button
            ...
            className={`px-4 py-2.5 rounded-xl border text-xs font-bold transition flex items-center gap-2 cursor-pointer shadow-md ${
              showMapView
          ...
          >
            ...
            <span>{showMapView ? (language === 'bn' ? 'গ্রিড ভিউ' : 'Grid View') : (language === 'bn' ? '🗺️ লাইভ ম্যাপ ভিউ' : '🗺️ Live Map View')}</span>
          </button>
      ...
      {/* Real-time Search Box (Debounced 300ms) */}
      ...
      {showMapView && (
        <NearbyDonorsMap
          ...
          onSelectDonor={(d: any) => setSelectedDonor(d)}
        />
      )}
      ...
      {loading ? (
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {donors.map(donor => {
            const isFav = favorites.includes(donor.donorId);
            const donorCoord = getCoordinatesForLocation(donor.latitude, donor.longitude, donor.district, donor.division);
            const distKm = userLat && userLng ? calculateHaversineDistance(userLat, userLng, donorCoord.lat, donorCoord.lng) : null;
            const repBadge = getReputationBadge(calculateReputationScore(donor, donor.totalDonations));
              ...
              >
                ...
                <div className="flex justify-between items-start gap-3">
                  <div className="flex items-center gap-3">
                    ...
                    <div className="space-y-0.5">
                      <div className="flex items-center gap-1.5 flex-wrap">
                        ...
                        <span className={`text-[9px] px-1.5 py-0.5 rounded font-bold ${repBadge.bgColor} ${repBadge.color} ${repBadge.borderColor} border`}>
                          {repBadge.badgeEmoji} {repBadge.tier}
                        </span>
                      </div>
                      ...
                      {distKm !== null && (
                        <p className="text-[10px] text-emerald-400 font-bold font-mono pt-0.5">
                          📍 {formatDistance(distKm, language)}
                        </p>
                      )}
                    </div>
                  </div>
                  ...
                  <div className="flex flex-col items-end gap-2">
                    ...
                    <button
                      onClick={(e) => handleToggleFavorite(donor.donorId, e)}
                      className={`p-1.5 rounded-full border transition cursor-pointer ${
                        isFav
                          ? 'bg-rose-500/20 border-rose-500/40 text-rose-500'
                          : 'bg-slate-950/80 border-slate-800 text-slate-500 hover:text-rose-400'
                      }`}
