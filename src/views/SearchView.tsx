...
import { Search, UserCheck, Phone, Check, MapPin, Calendar, Heart, ShieldAlert, Copy, AlertTriangle, RefreshCw, X, Facebook, ExternalLink, ShieldCheck } from 'lucide-react';
...
const getGenderLabel = (d: User, language: 'en' | 'bn') => {
  if (d.gender === 'female') {
...
export default function SearchView({ currentUser, initialFilters, onFiltersChange }: SearchViewProps) {
  const { language, t, formatLocation } = useLanguage();
  ...
  const fetchDonors = async (isManualRefresh = false) => {
    setLoading(true);
    setError(null);
    try {
      ...
      if (isManualRefresh) {
        ...
        const msg = language === 'bn' 
          ? `রক্তদাতা তালিকা সফলভাবে রিফ্রেশ করা হয়েছে (${count} জন সংসংগতিপূর্ণ)`
          : `Donor directory refreshed successfully! (${count} matching donor${count === 1 ? '' : 's'} found)`;
        showToast(msg, 'success');
      }
    } catch (err: any) {
      const errorMsg = err.message || (language === 'bn' ? 'রক্তদাতা অনুসন্ধান ব্যর্থ হয়েছে।' : 'Failed to search donors.');
      setError(errorMsg);
      if (isManualRefresh) {
    ...
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-10 space-y-10 relative">
      ...
      <AnimatePresence>
        {toast && (
          ...
          >
            <div className={`p-4 rounded-2xl border backdrop-blur-md flex items-center justify-between gap-3 text-xs font-bold shadow-xl ${
              toast.type === 'success' 
      ...
      <div className="text-left">
        <h1 className="text-2xl font-extrabold text-slate-100 tracking-tight">{language === 'bn' ? 'রক্তদাতা অনুসন্ধান কেন্দ্র' : 'Smart Voluntary Donor Directory'}</h1>
        <p className="text-xs text-slate-400">{language === 'bn' ? 'বাংলাদেশের ৬৪ জেলা ও উপেজলায় যাচাইকৃত রক্তদাতা খুঁজুন।' : 'Search verified voluntary donors in Bangladesh with high-precision location filtering.'}</p>
      </div>
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 pt-2 border-t border-slate-800/40">
          ...
          >
            ...
            {loading 
              ...
              : (language === 'bn' ? 'রিফ্রেশ তালিকা' : 'Refresh Roster')
            }
          </button>
        </div>
      </div>
      ...
      <div className="space-y-6">
        <div className="flex justify-between items-center text-xs">
          <span className="text-slate-400">Search returned <strong className="text-rose-400">{donors.length}</strong> matching donors</span>
            ...
            {donors.map((donor) => (
              ...
              >
                ...
                <div className="flex justify-between items-start gap-3">
                  <div className="flex items-center gap-3">
                    ...
                    {donor.avatarUrl ? (
                      ...
                      <div className="w-12 h-12 rounded-full bg-rose-500/10 border border-rose-500/20 text-rose-400 flex items-center justify-center font-extrabold text-sm uppercase shrink-0 shadow-inner">
                        {donor.name ? donor.name.split(' ').map(n => n[0]).slice(0, 2).join('') : 'U'}
                      </div>
                    )}
                    
                    <div className="space-y-0.5">
                      <h4 className="text-xs font-bold text-slate-100 group-hover:text-rose-400 transition-colors line-clamp-1">{donor.name}</h4>
                  ...
                  {/* Availability Badge */}
                  <span className={`text-[9px] font-bold px-2 py-0.5 rounded-full uppercase shrink-0 border tracking-wider ${
                    donor.isAvailable 
                      ? 'bg-emerald-500/10 text-emerald-400 border-emerald-500/20' 
                      : 'bg-slate-800/85 text-slate-500 border-slate-800'
                  }`}>
                    {donor.isAvailable ? (language === 'bn' ? 'প্রস্তুত' : 'Available') : (language === 'bn' ? 'বিশ্রামে' : 'Resting')}
                  </span>
                </div>
                ...
                <div className="space-y-2 text-xs text-slate-300 bg-slate-950/45 p-3.5 rounded-xl border border-slate-800/60">
                  {/* Verification Status */}
                  {(donor.isVerified || donor.isDonorVerified || donor.verificationStatus === 'approved') && (
                    ...
                    <div className="flex flex-col gap-0.5 col-span-2 border-t border-slate-800/30 pt-1.5">
                      ...
                      <span className="text-xs font-extrabold text-slate-200">
                        {(donor as any).totalDonations !== undefined ? (
                          language === 'bn' ? (
                            `${(donor as any).totalDonations.toString().replace(/\d/g, (d: string) => '০১২৩৪৫৬৭৮৯'[parseInt(d, 10)])} বার`
                          ) : (
                            `${(donor as any).totalDonations} ${(donor as any).totalDonations === 1 ? 'time' : 'times'}`
                          )
                        ) : (
                          language === 'bn' ? '০ বার' : '0 times'
                        )}
                      </span>
                    </div>
                ...
                <div className="flex gap-2">
                  <button
                    ...
                    className="flex-1 bg-slate-950 hover:bg-slate-800 text-rose-400 hover:text-rose-300 text-xs font-bold py-2.5 rounded-xl border border-slate-800 hover:border-slate-700 transition flex items-center justify-center gap-1.5"
                  >
                    <Phone className="w-3.5 h-3.5" />
                    {language === 'bn' ? 'যোগাযোগ করুন' : 'Request Contact'}
                  </button>
                  {donor.facebookUrl && donor.showFacebook !== false && (
                    <a
                      href={donor.facebookUrl.startsWith('http') ? donor.facebookUrl : `https://${donor.facebookUrl.includes('facebook.com') ? '' : 'www.facebook.com/'}${donor.facebookUrl}`}
                      target="_blank"
          ...
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
            ...
            >
              ...
              <div className="space-y-2">
                <p className="text-[10px] font-bold uppercase tracking-wider text-slate-500">Verified Contact Number</p>
                <div className="flex items-center justify-between p-3.5 bg-slate-950 rounded-xl border border-slate-800">
                  <span className="text-base font-extrabold text-slate-200 select-all tracking-wide">{selectedDonor.phone}</span>
                  <div className="flex gap-1.5">
                    ...
                    <a
                      ...
                      className="p-2 text-white bg-rose-600 hover:bg-rose-500 rounded-lg transition shadow-md flex items-center justify-center"
                      title="Dial Phone"
                    >
              ...
              {selectedDonor.facebookUrl && selectedDonor.showFacebook !== false && (
                <div className="space-y-2">
                  ...
                  <a
                    href={selectedDonor.facebookUrl.startsWith('http') ? selectedDonor.facebookUrl : `https://${selectedDonor.facebookUrl.includes('facebook.com') ? '' : 'www.facebook.com/'}${selectedDonor.facebookUrl}`}
                    target="_blank"
                    rel="noopener noreferrer"
              ...
              <button
                onClick={() => setSelectedDonor(null)}
                className="w-full bg-slate-800 hover:bg-slate-700 text-slate-300 text-xs font-bold py-3 rounded-xl border border-slate-700/60 transition"
              >
                Close Profile
              </button>
            </motion.div>
