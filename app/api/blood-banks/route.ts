...
import { fallbackBloodBanks } from '@/src/data/defaultDirectories';
...
export async function GET(req: NextRequest) {
  try {
    ...
    try {
      ...
      console.error(
        '[GET /api/blood-banks] Query: SELECT * FROM blood_banks WHERE ... | Error:',
        dbErr?.message || dbErr,
        '\nStack trace:',
        dbErr?.stack || 'No stack trace'
      );
      results = fallbackBloodBanks;
    }

    if (!results || results.length === 0) {
      let filteredFallback = [...fallbackBloodBanks];
      if (division) {
        filteredFallback = filteredFallback.filter(
          (b) => b.division.toLowerCase() === division.toLowerCase()
        );
      }
      if (search) {
    ...
    console.error(
      '[GET /api/blood-banks] Unexpected route exception | Stack trace:',
