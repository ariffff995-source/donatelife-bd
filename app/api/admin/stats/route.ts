...
export async function GET(req: NextRequest) {
  try {
    ...
    return NextResponse.json({
      ...
      totalHospitals: Number(totalHospitalsRes.rows[0]?.count || 0),
      totalBloodBanks: Number(totalBloodBanksRes.rows[0]?.count || 0),
      successfulDonations: Number(totalDonationsRes.rows[0]?.count || 0) + allReqs.filter((r) => r.status === 'fulfilled').length,
